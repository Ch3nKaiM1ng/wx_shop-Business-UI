...
<script>
import { addImg, modifyImg, finImg } from '@/api/navigation'
...
export default {
  ...
  methods: {
      ...
      if (
        ...
        if (this.id != null) {
          modifyImg({
            ...
            this.$message('修改成功')
            this.$router.push({
              path: '/advertising/centralSection'
            })
          })
        } else {
          console.log(self.__ob__.value)
          addImg(self.__ob__.value).then(res => {
            this.showNavList = res
            this.$message('添加成功')
            this.$router.push({
              path: '/advertising/centralSection'
    ...
    // 取消按钮
    onCancel() {
      this.$router.push({ path: '/advertising/centralSection' })
    }
  }
}
</script>
